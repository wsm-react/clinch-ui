    ...
    return (
        <div className="border-b border-t border-slate-300 dark:border-slate-800 mt-[-1px] group">
            <Link href={cardProps.href} className={cn("transition-all focus-visible:ring-[none] pt-6 pb-6 flex flex-row items-center justify-between", button === true ? "rounded-full p-6 pt-3 pb-3" : "")}>
                <div className="asass">
                    <div className={cn("sm:text-[1.2rem] font-medium group-hover:text-blue-700 dark:text-white dark:group-hover:text-lime-300 transition-all", isActive && "text-blue-700")}>{cardProps.title}</div>
                    <div className="sm:text-[.95rem] text-[.9rem] mt-1 sm:mt-0 dark:group-hover:text-gray-400 transition-all">{cardProps.label}</div>
                </div>
                <div className="w-10 h-8 ml-8">
                    <ChevronRight strokeWidth={.8} className="inline-block w-8 h-8 group-hover:text-blue-700 dark:group-hover:text-lime-300 transition-all group-hover:translate-x-3" />
                </div>
            </Link>
        </div>
    );
